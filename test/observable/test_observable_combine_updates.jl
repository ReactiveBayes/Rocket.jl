...
@testset "CombineLatestUpdatesObservable" begin
    ...
    run_testset([
        ...
        (
            source = combineLatestUpdates(
                faulted(Float64, "err1"),
                faulted(Float64, "err1"),
            ),
            values = @ts(e("err1")),
            source_type = Tuple{
                typeof(faulted(Float64, "err1")),
                typeof(faulted(Float64, "err1")),
            },
        ),
        (
    ...
    @testset begin
        ...
        combined = combineLatestUpdates(
            (source1, source2),
            PushNew(),
            String,
            (sources) -> string(sum(Rocket.getrecent.(sources))),
            callbackFn,
        )
        values = []
