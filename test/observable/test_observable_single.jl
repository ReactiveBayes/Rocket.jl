...
@testset "SingleObservable" begin
    ...
    @testset begin
        ...
        @test of([1, 2, 3]) ==
              SingleObservable{Vector{Int},AsapScheduler}([1, 2, 3], AsapScheduler())
        @test of((1, 2, 3)) ==
              SingleObservable{Tuple{Int,Int,Int},AsapScheduler}((1, 2, 3), AsapScheduler())
        @test of("Hello, world!") ==
              SingleObservable{String,AsapScheduler}("Hello, world!", AsapScheduler())
        @test of('H') == SingleObservable{Char,AsapScheduler}('H', AsapScheduler())
        @test of('H') == of('H')
        @test of(0) != of(0.0)

        @test of(1, scheduler = DummyScheduler()) ==
              SingleObservable{Int,DummyScheduler}(1, DummyScheduler())
    end
    ...
    run_testset([
        (source = of(1), values = @ts([1, c]), source_type = Int),
        (source = of([1, 2, 3]), values = @ts([[1, 2, 3], c]), source_type = Vector{Int}),
        (
            source = of((1, 1.0)),
            values = @ts([(1, 1.0), c]),
