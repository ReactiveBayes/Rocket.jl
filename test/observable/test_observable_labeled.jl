...
@testset "LabeledObservable" begin
    ...
    @testset begin
        @test labeled(Val((:x,)), from([(1,), (2,), (3,)])) isa
              LabeledObservable{NamedTuple{(:x,),Tuple{Int}}}
        @test labeled(Val((:x, :y)), from([(1, 1.0), (2, 2.0), (3, 3.0)])) isa
              LabeledObservable{NamedTuple{(:x, :y),Tuple{Int,Float64}}}
    end
