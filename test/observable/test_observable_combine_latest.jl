...
@testset "CombineLatestObservable" begin
    ...
    run_testset([
        ...
        (
            ...
            values = @ts(
                [(1, 2.0)] ~ [(2, 2.0)] ~ [(3, 2.0)] ~ [(4, 2.0)] ~ [(5, 2.0)] ~ c
            ),
            source_type = Tuple{Int,Float64},
        ),
        (
        ...
        (
            source = combineLatest(
                faulted(Float64, "err1"),
                faulted(Float64, "err2"),
                strategy = PushNew(),
            ),
            values = @ts(e("err1")),
            source_type = Tuple{Float64,Float64},
        ),
        (source = combineLatest((), PushNew()), values = @ts(c), source_type = Tuple{}),
        (source = combineLatest((), PushEach()), values = @ts(c), source_type = Tuple{}),
    ])

    somenumbers = [[1 for i = 1:50]..., [1.0 for i = 1:50]...]
    ...
    run_testset([(
        source = combined100,
        values = @ts([tuple([[1 for i = 1:50]..., [1.0 for i = 1:50]...]...), c]),
    ...
    @testset begin
        ...
        @test values ==
              [(1, 2.0, "Hello"), (1, 3.0, "Hello, world!"), (4, 4.0, ""), (4, 5.0, "foo")]
    ...
    @testset begin
        ...
        latest = combineLatest(
            ...
            s2,
            s3,
            strategy = PushStrategy(BitArray([false, true, false])),
        )
        values = Vector{Any}()
        subscription = subscribe!(
            latest,
        ...
        @test values ==
              [(1, 2.0, "Hello"), (2, 3.0, "Hello, world!"), (4, 4.0, ""), (4, 5.0, "1")]
