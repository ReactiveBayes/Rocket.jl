...
@testset "operator: merge_map()" begin
    ...
    run_testset([
        ...
        (
            source      = from(1:5) |> merge_map(Float64, _ -> throwError(Float64, "err")),
            values      = @ts(e("err")),
            source_type = Float64
        ),
        (
        ...
        (
            source      = throwError(Int, "e") |> merge_map(String, d -> string(d)),
            values      = @ts(e("e")),
            source_type = String
        ),
        (
        ...
        (
            source      = from([ of(1), throwError(Int, "err"), of(2) ]) |> merge_map(Int),
            values      = @ts([ 1, e("err") ]),
            source_type = Int
        ),
        (
            source      = from([ of(1), throwError(Int, "err"), of(2) ]) |> async(0) |> merge_map(Int),
            values      = @ts([ 1 ] ~ e("err")),
            source_type = Int
        ),
        (
