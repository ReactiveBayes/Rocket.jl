...
@testset "operator: async()" begin
    ...
    run_testset([
        ...
        (
            source = from_iterable(1:5) |> async(),
            values = @ts(0 ~ [ 1, 2, 3, 4, 5, c ])
        ),
        (
            source = from_iterable(1:5) |> async(0),
            values = @ts([1] ~ [2] ~ [3] ~ [4] ~ [5] ~ c)
        ),
        (
            source = from_iterable(1:5) |> async(1),
            values = @ts([1] ~ [2] ~ [3] ~ [4] ~ [5] ~ c)
        ),
        (
            source = from_iterable(1:5) |> async(2),
            values = @ts([ 1, 2 ] ~ [ 3, 4 ] ~ [ 5, c ])
        ),
        (
            source = from_iterable(1:5) |> async(3),
            values = @ts([ 1, 2, 3 ] ~ [ 4, 5, c ])
        ),
        (
