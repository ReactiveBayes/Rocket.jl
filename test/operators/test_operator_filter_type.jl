...
@testset "operator: filter_type()" begin
    ...
    run_testset([
        ...
        (
            source = from(Union{Int,Float64}[1, 2.0, 3, 4.0, 5, 6.0]) |>
                     filter_type(Float64),
            values = @ts([2.0, 4.0, 6.0, c]),
            source_type = Float64,
        ),
        (source = completed() |> filter_type(Int), values = @ts(c), source_type = Int),
        (
            source = faulted("e") |> filter_type(Int),
            values = @ts(e("e")),
            source_type = Int,
        ),
        (source = never() |> filter_type(Int), values = @ts(), source_type = Int),
    ])
