...
@testset "operator: substitute()" begin
    ...
    run_proxyshowcheck(
        "Substitute",
        substitute(Any, identity, SubstituteHandler()),
    ...
    run_testset([
        ...
        (
            source = faulted(Int, "e") |>
                     substitute(String, i -> string(i), SubstituteHandler()),
            values = @ts(e("e")),
            source_type = String,
        ),
        (
        ...
        (
            source = from(1:5) |>
                     tap(_ -> release!(global_handler)) |>
                     substitute(Int, identity, global_handler),
            values = @ts([1, 1, 2, 3, 4, c]),
            source_type = Int,
        ),
        (
            source = from(1:5) |>
                     tap(i -> rem(i, 2) == 0 ? release!(global_handler) : nothing) |>
                     substitute(Int, identity, global_handler),
        ...
        (
            source = from(1:5) |>
                     tap(i -> rem(i, 2) == 0 ? release!(global_handler) : nothing) |>
                     substitute(String, string, global_handler),
            values = @ts(["1", "1", "1", "3", "3", c]),
            source_type = String,
        ),
        (
