...
@testset "operator: override()" begin
    ...
    run_testset([
        ...
        (
            source = from(1:5) |>
                     tap(i -> Rocket.setvalue!(global_handler1, i ^ 2)) |>
                     override(global_handler1),
            values = @ts([1, 4, 9, 16, 25, c]),
            source_type = Int,
        ),
        (
            source = from(1:5) |>
                     override(global_handler2) |>
                     tap(i -> Rocket.setvalue!(global_handler2, i)),
        ...
        (source = never() |> override(OverrideHandler(Char, 'a')), values = @ts()),
    ])
