...
@testset "Operator" begin
    ...
    Rocket.as_operator(::Type{<:ExplicitlyDefinedOperator}) =
        TypedOperatorTrait{String,Int}()

    struct IdentityIntOperator <: TypedOperator{Int,Int} end
    Rocket.on_call!(::Type{Int}, ::Type{Int}, operator::IdentityIntOperator, source) =
        source
    ...
    Rocket.on_call!(
        ::Type{String},
        ::Type{String},
        operator::LeftTypedStringIdentityOperator,
        source,
    ) = source
    ...
    Rocket.on_call!(
        ::Type{Int},
        ::Type{Int},
        operator::LeftTypedIntIdentityNotImplementedOperator,
        source,
    ) = source
    ...
    Rocket.on_call!(
        ::Type{L},
        ::Type{Float64},
        operator::RightTypedFloatZeroOperator,
        source,
    ) where {L} = Rocket.from([0.0])

    struct InferableIntoZeroTupleOperator <: InferableOperator end
    Rocket.on_call!(
        ::Type{L},
        ::Type{Tuple{L,L}},
        operator::InferableIntoZeroTupleOperator,
        source,
    ) where {L} = Rocket.from([(zero(L), zero(L))])
    Rocket.operator_right(::InferableIntoZeroTupleOperator, ::Type{L}) where {L} =
        Tuple{L,L}
    ...
    @testset "as_operator" begin
        ...
        @test as_operator(LeftTypedStringIdentityOperator) ===
              LeftTypedOperatorTrait{String}()
        @test as_operator(RightTypedFloatZeroOperator) ===
    ...
    @testset "|>" begin
        ...
        @test_throws InconsistentSourceOperatorDataTypesError string_source |>
                                                              IdentityIntOperator()
        @test_throws InconsistentSourceOperatorDataTypesError int_source |>
                                                              LeftTypedStringIdentityOperator()

        # Check if pipe operator throws an error for not implemented operator
        @test_throws MissingOnCallImplementationError string_source |>
                                                      ExplicitlyDefinedOperator()
        @test_throws MissingOnCallImplementationError int_source |> NotImplementedOperator()

        @test_throws MissingOperatorRightImplementationError int_source |>
                                                             LeftTypedIntIdentityNotImplementedOperator()
        @test_throws MissingOperatorRightImplementationError int_source |>
                                                             InferableNotImplementedOperator()
