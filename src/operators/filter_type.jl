...
on_call!(::Type{L}, ::Type{T}, operator::FilterTypeOperator{T}, source) where {L,T<:L} =
    proxy(T, source, FilterTypeProxy{L,T}())
on_call!(::Type{L}, ::Type{T}, operator::FilterTypeOperator{T}, source) where {L,T} = error(
    "filter_type(T) operator usage error. Cannot filter $(T) elements from stream of $(L) elements.",
)

operator_right(::FilterTypeOperator{T}, ::Type{T}) where {T} = T
operator_right(::FilterTypeOperator{T}, ::Type{L}) where {L,T<:L} = T
operator_right(::FilterTypeOperator{T}, ::Type{L}) where {L,T} = error(
    "filter_type(T) operator creation error. Cannot filter $(T) elements from stream of $(L) elements.",
)

struct FilterTypeProxy{L,T} <: ActorProxy end

actor_proxy!(::Type{T}, proxy::FilterTypeProxy{L,T}, actor::A) where {L,A,T} =
    FilterTypeActor{L,A,T}(actor)
