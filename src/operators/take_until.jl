...
source_proxy!(::Type{L}, proxy::TakeUntilProxy{N}, source::S) where {L,N,S} =
    TakeUntilSource{L,N,S}(proxy.notifier, source)
...
mutable struct TakeUntilInnerActor{L,A} <: Actor{L}
    ...
    TakeUntilInnerActor{L,A}(actor::A) where {L,A} =
        new(actor, false, voidTeardown, voidTeardown)
end
