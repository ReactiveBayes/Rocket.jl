...
"""
concat_map(::Type{R}, mappingFn::F = identity) where {R,F<:Function} =
    ConcatMapOperator{R,F}(mappingFn)
...
function on_call!(
    ::Type{L},
    ::Type{R},
    operator::ConcatMapOperator{R,F},
    source,
) where {L,R,F}
...
actor_proxy!(::Type{R}, proxy::ConcatMapProxy{L,R,F}, actor::A) where {L,R,F,A} =
    ConcatMapActor{L,R,F,A}(proxy.mappingFn, actor)
...
isicompleted(actor::ConcatMapActor) =
    getactive(actor) === nothing && length(getpending(actor)) === 0
...
setsubscription!(actor::ConcatMapInnerActor, subscription) =
...
source_proxy!(::Type{R}, proxy::ConcatMapProxy{L,R,F}, source::S) where {L,R,F,S} =
    ConcatMapSource{L,S}(source)
