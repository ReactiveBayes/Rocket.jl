...
"""
exhaust_map(::Type{R}, mappingFn::F = identity) where {R,F<:Function} =
    ExhaustMapOperator{R,F}(mappingFn)
...
function on_call!(
    ::Type{L},
    ::Type{R},
    operator::ExhaustMapOperator{R,F},
    source,
) where {L,R,F}
...
actor_proxy!(::Type{R}, proxy::ExhaustMapProxy{L,R,F}, actor::A) where {L,R,F,A} =
    ExhaustMapActor{L,R,F,A}(proxy.mappingFn, actor)
...
mutable struct ExhaustMapActor{L,R,F,A} <: Actor{L}
    ...
    ExhaustMapActor{L,R,F,A}(mappingFn::F, actor::A) where {L,R,F,A} =
...
function on_next!(actor::S, data::L) where {L,R,S<:ExhaustMapActor{L,R}}
    if !isdisposed(actor)
        if isicompleted(actor)
            ...
            actor.isubscription =
                subscribe!(actor.mappingFn(data), ExhaustMapInnerActor{R,S}(actor))
        end
    end
end
...
source_proxy!(::Type{R}, proxy::ExhaustMapProxy{L,R,F}, source::S) where {L,R,F,S} =
    ExhaustMapSource{L,S}(source)
...
Base.show(io::IO, ::ExhaustMapActor{L,R}) where {L,R} =
    print(io, "ExhaustMapActor($L -> $R)")
Base.show(io::IO, ::ExhaustMapInnerActor{R}) where {R} =
    print(io, "ExhaustMapInnerActor($R)")
