...
actor_proxy!(::Type{Tuple{T, L}}, proxy::TupleWithLeftProxy{T}, actor::A) where { T, L, A } = TupleWithLeftActor{T, L, A}(proxy.value, actor)
...
Base.show(io::IO, ::TupleWithLeftActor{T, L}) where { T, L } = print(io, "TupleWithLeftActor($L -> Tuple{$T, $L})")
...
actor_proxy!(::Type{Tuple{L, T}}, proxy::TupleWithRightProxy{T}, actor::A) where { T, L, A } = TupleWithRightActor{T, L, A}(proxy.value, actor)
