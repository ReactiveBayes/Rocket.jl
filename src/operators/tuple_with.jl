...
actor_proxy!(proxy::TupleWithLeftProxy{T, L}, actor::A) where { T, L, A } = TupleWithLeftActor{T, L, A}(proxy.value, actor)
...
Base.show(io::IO, ::TupleWithLeftProxy{T, L}) where { T, L } = print(io, "TupleWithLeftProxy($L)")
Base.show(io::IO, ::TupleWithLeftActor{T, L}) where { T, L } = print(io, "TupleWithLeftActor($L)")
...
function on_call!(::Type{L}, ::Type{Tuple{L, T}}, operator::TupleWithRightOperator{T}, source) where T where L
    return proxy(Tuple{L, T}, source, TupleWithRightProxy{T, L}(operator.value))
end
...
actor_proxy!(proxy::TupleWithRightProxy{T, L}, actor::A) where { T, L, A } = TupleWithRightActor{T, L, A}(proxy.value, actor)
...
Base.show(io::IO, ::TupleWithRightProxy{T, L}) where { T, L } = print(io, "TupleWithRightProxy($L)")
Base.show(io::IO, ::TupleWithRightActor{T, L}) where { T, L } = print(io, "TupleWithRightActor($L)")
