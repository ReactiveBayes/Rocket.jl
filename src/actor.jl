...
error!(actor::T, error) where T = actor_on_error!(as_actor(T), actor, error)
...
actor_on_error!(::UndefinedActorTrait,  actor, error) = error("Type $(typeof(actor)) is not a valid actor type. \nConsider extending your actor with one of the abstract actor types <: (Actor{T}, NextActor{T}, ErrorActor{T}, CompletionActor{T}).")
actor_on_error!(::BaseActorTrait,       actor, error) = on_error!(actor, error)
actor_on_error!(::NextActorTrait,       actor, error) = begin end
actor_on_error!(::ErrorActorTrait,      actor, error) = on_error!(actor, error)
actor_on_error!(::CompletionActorTrait, actor, error) = begin end
...
on_error!(actor, error) = error("You probably forgot to implement on_error!(actor::$(typeof(actor)), error).")
