...
create_actor(::Type{L}, factory::FunctionActorFactory{F}) where {L,F} =
    FunctionActor{L,F}(factory.on_next)

subscribe!(source, fn::F) where {F<:Function} =
    subscribe!(source, FunctionActorFactory{F}(fn))
